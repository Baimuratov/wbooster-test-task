...
foreach ($arResult["ITEMS"] as &$arItem) {
	$arItem["CATEGORIES"] = CIBlockSection::GetNavChain(false, $arItem["IBLOCK_SECTION_ID"], ["NAME", "CODE"], true);

	$res = CIBlockElement::GetProperty($arItem["IBLOCK_ID"], $arItem["ID"], "sort", "asc", ["CODE" => "views"]);
	$arItem["VIEWS"] = $res->GetNext()["VALUE"];

	$arItem["READ_TIME"] = GetReadTime($arItem["DETAIL_TEXT"], 1);
}
